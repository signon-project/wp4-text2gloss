...
from text2gloss.rule_based.find_needed_clauses import find_verb_temp_ques
from text2gloss.rule_based.helpers import Clause, delete_item_sometimes
from text2gloss.rule_based.transform_comp import transform_meer_minder_dan
from text2gloss.rule_based.transform_conj import transform_conj
from text2gloss.rule_based.transform_neg import concatenate_may_can_not
from text2gloss.rule_based.transform_num import transform_numbers
from text2gloss.rule_based.transform_plural import transform_plural
from text2gloss.rule_based.transform_prep import transform_locative_prepositions, transform_prepositions
from text2gloss.rule_based.transform_pron_verb import (
    join_separable_verbs,
    join_separable_verbs2,
    possessive_pronouns,
    transform_pronoun_verb,
    transform_verb2,
)
from text2gloss.rule_based.transform_ques import transform_question_word_clause
from text2gloss.rule_based.transform_with_gloss_ids import match_with_gloss_ids
from text2gloss.rule_based.transform_word_specific import (
    genoeg,
    last_processing_step,
    lemma_not_infinitive,
    relative_clause_with_of,
    transform_other_mistakes,
    two_nouns,
    was,
)
from text2gloss.rule_based.word_lists import (
    ...
    blijven_conjugation,
    can_conjugation,
    comparison_with_separate_sign_with_e_del_e,
    have_conjugation,
    lijken_conjugation,
    locative_preps,
    locative_verbs,
    ...
    pers4,
    pers5,
    separate_negation_sign,
    seperated_part_of_verb,
    tw_to_ignore,
    worden_conjugation,
    words_without_gloss,
)
...
def dutch_to_gloss(sentence_list):
    ...
    subj_index = [
        ...
        if "subj" in sentence[index].dep_ and sentence[index].pos_ in ["NOUN", "PROPN", "PRON", "X", "NUM"]
    ]
    iobj_index = [
        index
        for index, token in enumerate(sentence)
        if sentence[index].dep_ == "iobj" and sentence[index].pos_ in ["NOUN", "PROPN", "PRON", "X", "NUM"]
    ]
    obj_index = [
        index
        for index, word in enumerate(sentence)
        if sentence[index].dep_ == "obj" and sentence[index].pos_ in ["NOUN", "PROPN", "PRON", "X", "NUM"]
    ]
    if not subj_index:
        subj_index = [
            index
            for index, token in enumerate(sentence)
            if "ROOT" in sentence[index].dep_ and ("N|" in sentence[index].tag_ or "VNW" in sentence[index].tag_)
        ]
    ...
    for index2, item2 in enumerate(sentence_object.clause_list):
        ...
        sentence[index2].new_form = (
            "WG-1"
            if item2.lemma_ in pers1
            else "WG-2"
            if item2.lemma_ in pers2
            else "WG-4"
            if item2.lemma_ in pers4
            else "WG-5"
            if item2.lemma_ in pers5
            else "WG-6"
            if item2.lemma_ in pers3_6 and "VNW" in item2.tag_ and "mv" in item2.tag_
            else "WG-3"
            if item2.lemma_ in pers3_6 and "VNW" in item2.tag_
        ...
        # # # NO SIGN
            ...
            or (
                item2.text == "meer"
                and ("niet" in [item3.text for item3 in sentence] or "geen" in [item4.text for item4 in sentence])
            )
        ):
