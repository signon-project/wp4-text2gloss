...
from word_lists import double_numbers, double_signs, one_options, small_units, bigger_units, big_units, \
    big_units_numbers, number_in_words, number_in_numbers
import re
import telwoord
...
def to_numbers(m, number_written_out):
    ...
    if not matched:
        ...
        for unit2 in small_units:
            m = re.sub(unit2, unit2 + ' ', m) if not re.search(rf"{unit2}t", m) \
                                                     and not re.search(rf'{unit2}i', m) else m  # tacht-ig & negen-tig
            m = re.sub(r' [eë]n', ' ', m)
...
def transform_numbers(sentence_object, index, item):
    ...
    number_list = [re.sub(r'\.', '', number)] if re.search(r'\.[0-9][0-9][0-9]', number) \
        else number.split('.') if re.search(r'[.]', number) else [number]
