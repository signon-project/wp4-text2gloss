...
from text2gloss.rule_based.word_lists import (
    elements_incorporated_in_time_sign,
    may_can,
    not_signed_verbs,
    preposition_with_time,
    question_words,
    question_words_with_noun,
    signed_if_only_verb,
    start_end,
...
def find_verbs(sentence_object):
    ...
    verbs = (
        [it for it in sentence if "WW" in it.tag_ and "prenom" not in it.tag_]
        if [it for it in sentence if "WW" in it.tag_ and "prenom" not in it.tag_]
        else [it for it in sentence if it.pos_ == "VERB" and "prenom" not in it.tag_]
        if [it for it in sentence if it.pos_ == "VERB" and "prenom" not in it.tag_]
        else []
    )
...
def find_question_phrase(sentence_object):
    ...
    if sentence_object.question_word_indices:
        ...
        if (
            sentence[sentence_object.question_word_indices[-1]].lemma_ in question_words_with_noun
...
def find_temp_clause(sentence_object):
    ...
    for index, item in enumerate(sentence):
        ...
        if (
            ("krijgen" in [item4.lemma_ for item4 in sentence] or "geven" in [item4.lemma_ for item4 in sentence])
            and item.dep_ == "obj"
            and item.lemma_ in time_unit
        ):
            ignore = True
        ...
        if (item.text in temporal_expressions or item.lemma_ in temporal_expressions) and not ignore:
            ...
            if temporal_clause_indices:
                temporal_clause_indices = [
                    index4 for index4 in range(min(temporal_clause_indices), max(temporal_clause_indices) + 1)
                ]
    ...
    temporal_clause_indices_new = [
        index
        for index in list(dict.fromkeys(temporal_clause_indices))
