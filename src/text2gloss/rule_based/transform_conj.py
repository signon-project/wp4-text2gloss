...
from text2gloss.rule_based.helpers import Word, delete_item_sometimes
from text2gloss.rule_based.word_lists import (
    conjunctions_because,
    conjunctions_but,
    conjunctions_if,
    conjunctions_then_before,
    not_used_conjunctions,
)
...
def transform_conj(sentence_object, index, item):
    ...
    if item.lemma_ in conjunctions_then_before:
        ...
        item.new_form = "MAAR"
        item.lemma_ = "maar"
    if item.lemma_ in conjunctions_then_before + conjunctions_because + conjunctions_if + conjunctions_but and not (
        item.lemma_ == "dan" and item.dep_ in ["mark", "fixed"]
    ):
        item.dep_ = "cc"
        item.tag_ = "VG|neven"
        item.pos_ = "CCONJ"
