...
from word_lists import demonstrative, have_conjugation, must_conjugation, can_conjugation, may_conjugation, \
    be_conjugation, worden_conjugation, lijken_conjugation, blijken_conjugation, blijven_conjugation
...
def two_nouns(sentence_object, index, item):
    ...
    sentence = sentence_object.clause_list
    if index > 1:
        if sentence[index-1].pos_ == 'NOUN' and not (sentence[index-1].dep_ == 'iobj' and item.dep_ == 'obj')\
                and item.tag_ == 'N|soort|mv|basis':
            item.head = sentence[index-1].text
        elif sentence[index-2].pos_ == 'NOUN' and sentence[index-1].head == item.text \
...
def lemma_not_infinitive(sentence_object, index, item):
    ...
    if 'WW' in item.tag_:
        item.lemma_ = 'kunnen' if item.lemma_ in can_conjugation else 'hebben' if item.lemma_ in have_conjugation \
            else 'moeten' if item.lemma_ in must_conjugation else 'mogen' if item.lemma_ in may_conjugation \
            else 'zijn' if item.lemma_ in be_conjugation else 'worden' if item.lemma_ in worden_conjugation \
            else 'lijken' if item.lemma_ in lijken_conjugation else 'blijven' if item.lemma_ in blijven_conjugation \
            else 'blijken' if item.lemma_ in blijken_conjugation else item.lemma_
        item.new_form = 'KUNNEN' if item.lemma_ in can_conjugation \
            else 'HEBBEN' if item.lemma_ in have_conjugation else 'MOETEN' if item.lemma_ in must_conjugation \
            else 'MOGEN' if item.lemma_ in may_conjugation else item.new_form
    return sentence_object
...
def richting(sentence, index, item):
    ...
    if index > 0 and len(sentence) > index + 1:
        if sentence[index - 1].pos_ != 'DET' \
                or (sentence[index - 2].lemma_ == 'in' and sentence[index + 1].lemma_ == 'van'):
            item.new_form = 'NAAR'
            item.text = 'naar'
            item.lemma_ = 'naar'
...
def transform_other_mistakes(sentence_object, index, item, all_preps):
    ...
    # VOLGENDE
    elif item.text == 'volgend' or item.text == 'volgende' and not 'WW' in item.tag_:
