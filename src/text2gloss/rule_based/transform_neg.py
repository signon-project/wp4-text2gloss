"""handle negation"""
from text2gloss.rule_based.word_lists import can_conjugation, may_conjugation
...
def no_negation_in_question(sentence, sentence_new, index, item):
    ...
    if (
        item.lemma_ == "niet"
        and sentence.pop().lemma_ == "?"
        and "als" not in [item5.lemma_ for item5 in sentence]
        and "indien" not in [item6.lemma_ for item6 in sentence]
    ):
